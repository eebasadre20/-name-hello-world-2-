...
export const generateTokens = (managerId: string, rememberInHours: number) => {
  ...
  const accessTokenExpiresIn = 24 * 60 * 60; // 24 hours in seconds (as per requirement)
  const refreshTokenExpiresIn = 48 * 60 * 60; // Refresh token should expire in 48 hours, not based on rememberInHours
  ...
  const refresh_token = sign( // Refresh token should expire in 48 hours, not based on rememberInHours
    { 
      id: manager.id, 
      email: manager.email,
    },
